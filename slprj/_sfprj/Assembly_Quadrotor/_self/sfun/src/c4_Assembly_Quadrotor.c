...
#include "c4_Assembly_Quadrotor.h"
...
static const char * c4_debug_family_names[4] = { "nargin", "nargout", "AngleDeg",
  "AngleFix" };

/* Function Declarations */
static void initialize_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static void initialize_params_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static void enable_c4_Assembly_Quadrotor(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static void disable_c4_Assembly_Quadrotor(SFc4_Assembly_QuadrotorInstanceStruct *
  chartInstance);
static void c4_update_debugger_state_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static void set_sim_state_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance, const mxArray *c4_st);
static void finalize_c4_Assembly_Quadrotor(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static void sf_gateway_c4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static void initSimStructsc4_Assembly_Quadrotor
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c4_machineNumber, uint32_T
  c4_chartNumber, uint32_T c4_instanceNumber);
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData);
static real_T c4_emlrt_marshallIn(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c4_AngleFix, const char_T *c4_identifier);
static real_T c4_b_emlrt_marshallIn(SFc4_Assembly_QuadrotorInstanceStruct
...
static real_T c4_mod(SFc4_Assembly_QuadrotorInstanceStruct *chartInstance,
                     real_T c4_x, real_T c4_y);
static void c4_eml_scalar_eg(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static const mxArray *c4_b_sf_marshallOut(void *chartInstanceVoid, void
  *c4_inData);
static int32_T c4_c_emlrt_marshallIn(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void c4_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData);
static uint8_T c4_d_emlrt_marshallIn(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c4_b_is_active_c4_Assembly_Quadrotor, const
  char_T *c4_identifier);
static uint8_T c4_e_emlrt_marshallIn(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c4_u, const emlrtMsgIdentifier *c4_parentId);
static void init_dsm_address_info(SFc4_Assembly_QuadrotorInstanceStruct
  *chartInstance);
  ...
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  chartInstance->c4_is_active_c4_Assembly_Quadrotor = 0U;
}
  ...
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  real_T *c4_AngleFix;
  c4_AngleFix = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c4_st = NULL;
  c4_st = NULL;
  c4_y = NULL;
  sf_mex_assign(&c4_y, sf_mex_createcellmatrix(2, 1), false);
  c4_hoistedGlobal = *c4_AngleFix;
  c4_u = c4_hoistedGlobal;
  c4_b_y = NULL;
  sf_mex_assign(&c4_b_y, sf_mex_create("y", &c4_u, 0, 0U, 0U, 0U, 0), false);
  sf_mex_setcell(c4_y, 0, c4_b_y);
  c4_b_hoistedGlobal = chartInstance->c4_is_active_c4_Assembly_Quadrotor;
  ...
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance, const mxArray *c4_st)
{
  ...
  *c4_AngleFix = c4_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell
    (c4_u, 0)), "AngleFix");
  chartInstance->c4_is_active_c4_Assembly_Quadrotor = c4_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c4_u, 1)),
     "is_active_c4_Assembly_Quadrotor");
  sf_mex_destroy(&c4_u);
  c4_update_debugger_state_c4_Assembly_Quadrotor(chartInstance);
  ...
  (SFc4_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  real_T c4_b_x;
  real_T c4_y;
  real_T *c4_b_AngleDeg;
  real_T *c4_b_AngleFix;
  c4_b_AngleFix = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c4_b_AngleDeg = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = sf_get_time(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 1U, chartInstance->c4_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c4_b_AngleDeg, 0U);
  chartInstance->c4_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 1U, chartInstance->c4_sfEvent);
  c4_hoistedGlobal = *c4_b_AngleDeg;
  c4_AngleDeg = c4_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c4_nargout, 1U, c4_sf_marshallOut,
    c4_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c4_AngleDeg, 2U, c4_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c4_AngleFix, 3U, c4_sf_marshallOut,
    c4_sf_marshallIn);
  CV_EML_FCN(0, 0);
  _SFD_EML_CALL(0U, chartInstance->c4_sfEvent, 3);
  if (CV_EML_IF(0, 1, 0, c4_AngleDeg > 0.0)) {
    _SFD_EML_CALL(0U, chartInstance->c4_sfEvent, 4);
    c4_AngleFix = c4_mod(chartInstance, c4_AngleDeg, 360.0);
  } else {
    _SFD_EML_CALL(0U, chartInstance->c4_sfEvent, 6);
    c4_x = c4_AngleDeg;
    c4_b_x = c4_x;
    c4_y = muDoubleScalarAbs(c4_b_x);
    c4_AngleFix = -c4_mod(chartInstance, c4_y, 360.0);
  }

  _SFD_EML_CALL(0U, chartInstance->c4_sfEvent, -6);
  _SFD_SYMBOL_SCOPE_POP();
  *c4_b_AngleFix = c4_AngleFix;
  ...
  _SFD_DATA_RANGE_CHECK(*c4_b_AngleFix, 1U);
}
...
static const mxArray *c4_sf_marshallOut(void *chartInstanceVoid, void *c4_inData)
{
  ...
  const mxArray *c4_y = NULL;
  SFc4_Assembly_QuadrotorInstanceStruct *chartInstance;
  chartInstance = (SFc4_Assembly_QuadrotorInstanceStruct *)chartInstanceVoid;
  ...
  *chartInstance, const mxArray *c4_AngleFix, const char_T *c4_identifier)
{
  ...
  c4_y = c4_b_emlrt_marshallIn(chartInstance, sf_mex_dup(c4_AngleFix),
    &c4_thisId);
  sf_mex_destroy(&c4_AngleFix);
  return c4_y;
}
...
static void c4_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c4_mxArrayInData, const char_T *c4_varName, void *c4_outData)
{
  const mxArray *c4_AngleFix;
  const char_T *c4_identifier;
  emlrtMsgIdentifier c4_thisId;
  real_T c4_y;
  SFc4_Assembly_QuadrotorInstanceStruct *chartInstance;
  chartInstance = (SFc4_Assembly_QuadrotorInstanceStruct *)chartInstanceVoid;
  c4_AngleFix = sf_mex_dup(c4_mxArrayInData);
...
const mxArray *sf_c4_Assembly_Quadrotor_get_eml_resolved_functions_info(void)
{
...
static void c4_info_helper(const mxArray **c4_info)
{
  ...
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("mod"), "name", "name", 0);
  ...
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut(
    "[ILXE]$matlabroot$/toolbox/eml/lib/matlab/elfun/eml_scalar_round.m"),
                  "resolved", "resolved", 8);
  sf_mex_addfield(*c4_info, c4_b_emlrt_marshallOut(1307672838U), "fileTimeLo",
                  "fileTimeLo", 8);
  sf_mex_addfield(*c4_info, c4_b_emlrt_marshallOut(0U), "fileTimeHi",
                  "fileTimeHi", 8);
  sf_mex_addfield(*c4_info, c4_b_emlrt_marshallOut(0U), "mFileTimeLo",
  ...
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut(
    "[ILXE]$matlabroot$/toolbox/eml/lib/matlab/elfun/mod.m!floatmod"), "context",
                  "context", 10);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("eps"), "name", "name", 10);
  ...
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut(
    "[ILXE]$matlabroot$/toolbox/eml/lib/matlab/elmat/eps.m"), "context",
                  "context", 12);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("eml_eps"), "name", "name", 12);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("char"), "dominantType",
                  "dominantType", 12);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut(
  ...
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("abs"), "name", "name", 14);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut("double"), "dominantType",
                  "dominantType", 14);
  sf_mex_addfield(*c4_info, c4_emlrt_marshallOut(
  ...
  char_T *c4_identifier)
{
  ...
  c4_y = c4_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c4_b_is_active_c4_Assembly_Quadrotor), &c4_thisId);
  sf_mex_destroy(&c4_b_is_active_c4_Assembly_Quadrotor);
  return c4_y;
}
...
void sf_c4_Assembly_Quadrotor_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c4_Assembly_Quadrotor_get_autoinheritance_info(void)
{
...
mxArray *sf_c4_Assembly_Quadrotor_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

mxArray *sf_c4_Assembly_Quadrotor_updateBuildInfo_args_info(void)
{
  mxArray *mxBIArgs = mxCreateCellMatrix(1,0);
  return mxBIArgs;
}

static const mxArray *sf_get_sim_state_info_c4_Assembly_Quadrotor(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[6],T\"AngleFix\",},{M[8],M[0],T\"is_active_c4_Assembly_Quadrotor\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c4_Assembly_Quadrotor_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    chartInstance = (SFc4_Assembly_QuadrotorInstanceStruct *)
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          real_T *c4_AngleFix;
          c4_AngleFix = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
          c4_AngleDeg = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
          _SFD_SET_DATA_VALUE_PTR(0U, c4_AngleDeg);
          _SFD_SET_DATA_VALUE_PTR(1U, c4_AngleFix);
        }
      }
    } else {
...
static void sf_opaque_initialize_c4_Assembly_Quadrotor(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
  initialize_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c4_Assembly_Quadrotor(void *chartInstanceVar)
{
  enable_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_disable_c4_Assembly_Quadrotor(void *chartInstanceVar)
{
  disable_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c4_Assembly_Quadrotor(void *chartInstanceVar)
{
  sf_gateway_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c4_Assembly_Quadrotor(SimStruct*
  S)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  ...
  prhs[2] = (mxArray*) get_sim_state_c4_Assembly_Quadrotor
    ((SFc4_Assembly_QuadrotorInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c4_Assembly_Quadrotor();/* state var info */
...
extern void sf_internal_set_sim_state_c4_Assembly_Quadrotor(SimStruct* S, const
  mxArray *st)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  ...
  prhs[2] = (mxArray*) sf_get_sim_state_info_c4_Assembly_Quadrotor();/* state var info */
  ...
  set_sim_state_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c4_Assembly_Quadrotor(SimStruct* S)
{
  return sf_internal_get_sim_state_c4_Assembly_Quadrotor(S);
}

static void sf_opaque_set_sim_state_c4_Assembly_Quadrotor(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c4_Assembly_Quadrotor(S, st);
}

static void sf_opaque_terminate_c4_Assembly_Quadrotor(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc4_Assembly_QuadrotorInstanceStruct*) chartInstanceVar)
      ->S;
    ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
    ...
    finalize_c4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc4_Assembly_Quadrotor((SFc4_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c4_Assembly_Quadrotor(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
    ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
    initialize_params_c4_Assembly_Quadrotor
      ((SFc4_Assembly_QuadrotorInstanceStruct*)(chartInfo->chartInstance));
  }
}

static void mdlSetWorkWidths_c4_Assembly_Quadrotor(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c4_Assembly_Quadrotor(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c4_Assembly_Quadrotor(SimStruct *S)
{
  SFc4_Assembly_QuadrotorInstanceStruct *chartInstance;
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)utMalloc(sizeof
    (ChartRunTimeInfo));
  chartInstance = (SFc4_Assembly_QuadrotorInstanceStruct *)utMalloc(sizeof
    (SFc4_Assembly_QuadrotorInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc4_Assembly_QuadrotorInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.disableChart =
    sf_opaque_disable_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.mdlStart = mdlStart_c4_Assembly_Quadrotor;
  chartInstance->chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c4_Assembly_Quadrotor;
...
void c4_Assembly_Quadrotor_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c4_Assembly_Quadrotor(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c4_Assembly_Quadrotor(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c4_Assembly_Quadrotor(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c4_Assembly_Quadrotor_method_dispatcher.\n"
