...
#include "c8_Assembly_Quadrotor.h"
...
static const char * c8_debug_family_names[5] = { "nargin", "nargout", "revSpeed",
  "revCnst", "Thrust" };

/* Function Declarations */
static void initialize_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static void initialize_params_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static void enable_c8_Assembly_Quadrotor(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static void disable_c8_Assembly_Quadrotor(SFc8_Assembly_QuadrotorInstanceStruct *
  chartInstance);
static void c8_update_debugger_state_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static void set_sim_state_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance, const mxArray *c8_st);
static void finalize_c8_Assembly_Quadrotor(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static void sf_gateway_c8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static void initSimStructsc8_Assembly_Quadrotor
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c8_machineNumber, uint32_T
  c8_chartNumber, uint32_T c8_instanceNumber);
static const mxArray *c8_sf_marshallOut(void *chartInstanceVoid, void *c8_inData);
static real_T c8_emlrt_marshallIn(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c8_Thrust, const char_T *c8_identifier);
static real_T c8_b_emlrt_marshallIn(SFc8_Assembly_QuadrotorInstanceStruct
...
static void c8_eml_scalar_eg(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance);
static const mxArray *c8_b_sf_marshallOut(void *chartInstanceVoid, void
  *c8_inData);
static int32_T c8_c_emlrt_marshallIn(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c8_u, const emlrtMsgIdentifier *c8_parentId);
static void c8_b_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c8_mxArrayInData, const char_T *c8_varName, void *c8_outData);
static uint8_T c8_d_emlrt_marshallIn(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c8_b_is_active_c8_Assembly_Quadrotor, const
  char_T *c8_identifier);
static uint8_T c8_e_emlrt_marshallIn(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance, const mxArray *c8_u, const emlrtMsgIdentifier *c8_parentId);
static void init_dsm_address_info(SFc8_Assembly_QuadrotorInstanceStruct
  *chartInstance);
  ...
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  chartInstance->c8_is_active_c8_Assembly_Quadrotor = 0U;
}
  ...
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  real_T *c8_Thrust;
  c8_Thrust = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c8_st = NULL;
  c8_st = NULL;
  c8_y = NULL;
  sf_mex_assign(&c8_y, sf_mex_createcellmatrix(2, 1), false);
  c8_hoistedGlobal = *c8_Thrust;
  c8_u = c8_hoistedGlobal;
  c8_b_y = NULL;
  sf_mex_assign(&c8_b_y, sf_mex_create("y", &c8_u, 0, 0U, 0U, 0U, 0), false);
  sf_mex_setcell(c8_y, 0, c8_b_y);
  c8_b_hoistedGlobal = chartInstance->c8_is_active_c8_Assembly_Quadrotor;
  ...
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance, const mxArray *c8_st)
{
  ...
  *c8_Thrust = c8_emlrt_marshallIn(chartInstance, sf_mex_dup(sf_mex_getcell(c8_u,
    0)), "Thrust");
  chartInstance->c8_is_active_c8_Assembly_Quadrotor = c8_d_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c8_u, 1)),
     "is_active_c8_Assembly_Quadrotor");
  sf_mex_destroy(&c8_u);
  c8_update_debugger_state_c8_Assembly_Quadrotor(chartInstance);
  ...
  (SFc8_Assembly_QuadrotorInstanceStruct *chartInstance)
{
  ...
  real_T c8_d_a;
  real_T c8_c;
  real_T *c8_b_revSpeed;
  real_T *c8_b_Thrust;
  real_T *c8_b_revCnst;
  c8_b_revCnst = (real_T *)ssGetInputPortSignal(chartInstance->S, 1);
  c8_b_Thrust = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
  c8_b_revSpeed = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
  _SFD_SYMBOL_SCOPE_PUSH(0U, 0U);
  _sfTime_ = sf_get_time(chartInstance->S);
  _SFD_CC_CALL(CHART_ENTER_SFUNCTION_TAG, 5U, chartInstance->c8_sfEvent);
  _SFD_DATA_RANGE_CHECK(*c8_b_revSpeed, 0U);
  chartInstance->c8_sfEvent = CALL_EVENT;
  _SFD_CC_CALL(CHART_ENTER_DURING_FUNCTION_TAG, 5U, chartInstance->c8_sfEvent);
  c8_hoistedGlobal = *c8_b_revSpeed;
  c8_b_hoistedGlobal = *c8_b_revCnst;
  c8_revSpeed = c8_hoistedGlobal;
  c8_revCnst = c8_b_hoistedGlobal;
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c8_nargout, 1U, c8_sf_marshallOut,
    c8_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c8_revSpeed, 2U, c8_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML(&c8_revCnst, 3U, c8_sf_marshallOut);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c8_Thrust, 4U, c8_sf_marshallOut,
    c8_sf_marshallIn);
  CV_EML_FCN(0, 0);
  ...
  c8_eml_scalar_eg(chartInstance);
  c8_c = c8_d_a * c8_d_a;
  c8_Thrust = c8_revCnst * c8_c;
  _SFD_EML_CALL(0U, chartInstance->c8_sfEvent, 4);
  if (CV_EML_IF(0, 1, 0, c8_Thrust > 0.0)) {
  ...
  *c8_b_Thrust = c8_Thrust;
  ...
  _SFD_DATA_RANGE_CHECK(*c8_b_Thrust, 1U);
  _SFD_DATA_RANGE_CHECK(*c8_b_revCnst, 2U);
}
...
static const mxArray *c8_sf_marshallOut(void *chartInstanceVoid, void *c8_inData)
{
  ...
  const mxArray *c8_y = NULL;
  SFc8_Assembly_QuadrotorInstanceStruct *chartInstance;
  chartInstance = (SFc8_Assembly_QuadrotorInstanceStruct *)chartInstanceVoid;
  ...
  *chartInstance, const mxArray *c8_Thrust, const char_T *c8_identifier)
{
  ...
  c8_y = c8_b_emlrt_marshallIn(chartInstance, sf_mex_dup(c8_Thrust), &c8_thisId);
  sf_mex_destroy(&c8_Thrust);
  return c8_y;
}
...
static void c8_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c8_mxArrayInData, const char_T *c8_varName, void *c8_outData)
{
  const mxArray *c8_Thrust;
  const char_T *c8_identifier;
  emlrtMsgIdentifier c8_thisId;
  real_T c8_y;
  SFc8_Assembly_QuadrotorInstanceStruct *chartInstance;
  chartInstance = (SFc8_Assembly_QuadrotorInstanceStruct *)chartInstanceVoid;
  c8_Thrust = sf_mex_dup(c8_mxArrayInData);
...
const mxArray *sf_c8_Assembly_Quadrotor_get_eml_resolved_functions_info(void)
{
...
static void c8_info_helper(const mxArray **c8_info)
{
  ...
  sf_mex_addfield(*c8_info, c8_emlrt_marshallOut(
    "[ILXE]$matlabroot$/toolbox/eml/lib/matlab/elmat/ismatrix.m"), "resolved",
                  "resolved", 2);
  sf_mex_addfield(*c8_info, c8_b_emlrt_marshallOut(1331326458U), "fileTimeLo",
                  "fileTimeLo", 2);
  sf_mex_addfield(*c8_info, c8_b_emlrt_marshallOut(0U), "fileTimeHi",
                  "fileTimeHi", 2);
  sf_mex_addfield(*c8_info, c8_b_emlrt_marshallOut(0U), "mFileTimeLo",
  ...
  char_T *c8_identifier)
{
  ...
  c8_y = c8_e_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c8_b_is_active_c8_Assembly_Quadrotor), &c8_thisId);
  sf_mex_destroy(&c8_b_is_active_c8_Assembly_Quadrotor);
  return c8_y;
}
...
void sf_c8_Assembly_Quadrotor_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c8_Assembly_Quadrotor_get_autoinheritance_info(void)
{
...
mxArray *sf_c8_Assembly_Quadrotor_third_party_uses_info(void)
{
  mxArray * mxcell3p = mxCreateCellMatrix(1,0);
  return(mxcell3p);
}

mxArray *sf_c8_Assembly_Quadrotor_updateBuildInfo_args_info(void)
{
  mxArray *mxBIArgs = mxCreateCellMatrix(1,0);
  return mxBIArgs;
}

static const mxArray *sf_get_sim_state_info_c8_Assembly_Quadrotor(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"Thrust\",},{M[8],M[0],T\"is_active_c8_Assembly_Quadrotor\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c8_Assembly_Quadrotor_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    chartInstance = (SFc8_Assembly_QuadrotorInstanceStruct *)
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
...

        {
          ...
          real_T *c8_Thrust;
          real_T *c8_revCnst;
          c8_revCnst = (real_T *)ssGetInputPortSignal(chartInstance->S, 1);
          c8_Thrust = (real_T *)ssGetOutputPortSignal(chartInstance->S, 1);
          c8_revSpeed = (real_T *)ssGetInputPortSignal(chartInstance->S, 0);
          _SFD_SET_DATA_VALUE_PTR(0U, c8_revSpeed);
          _SFD_SET_DATA_VALUE_PTR(1U, c8_Thrust);
          _SFD_SET_DATA_VALUE_PTR(2U, c8_revCnst);
        }
      }
    } else {
...
static void sf_opaque_initialize_c8_Assembly_Quadrotor(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
  initialize_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c8_Assembly_Quadrotor(void *chartInstanceVar)
{
  enable_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_disable_c8_Assembly_Quadrotor(void *chartInstanceVar)
{
  disable_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c8_Assembly_Quadrotor(void *chartInstanceVar)
{
  sf_gateway_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c8_Assembly_Quadrotor(SimStruct*
  S)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  ...
  prhs[2] = (mxArray*) get_sim_state_c8_Assembly_Quadrotor
    ((SFc8_Assembly_QuadrotorInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c8_Assembly_Quadrotor();/* state var info */
...
extern void sf_internal_set_sim_state_c8_Assembly_Quadrotor(SimStruct* S, const
  mxArray *st)
{
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
  ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
  ...
  prhs[2] = (mxArray*) sf_get_sim_state_info_c8_Assembly_Quadrotor();/* state var info */
  ...
  set_sim_state_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c8_Assembly_Quadrotor(SimStruct* S)
{
  return sf_internal_get_sim_state_c8_Assembly_Quadrotor(S);
}

static void sf_opaque_set_sim_state_c8_Assembly_Quadrotor(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c8_Assembly_Quadrotor(S, st);
}

static void sf_opaque_terminate_c8_Assembly_Quadrotor(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc8_Assembly_QuadrotorInstanceStruct*) chartInstanceVar)
      ->S;
    ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
    ...
    finalize_c8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc8_Assembly_Quadrotor((SFc8_Assembly_QuadrotorInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c8_Assembly_Quadrotor(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)(ssGetUserData(S));
    ChartInfoStruct * chartInfo = (ChartInfoStruct *)(crtInfo->instanceInfo);
    initialize_params_c8_Assembly_Quadrotor
      ((SFc8_Assembly_QuadrotorInstanceStruct*)(chartInfo->chartInstance));
  }
}

static void mdlSetWorkWidths_c8_Assembly_Quadrotor(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c8_Assembly_Quadrotor(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c8_Assembly_Quadrotor(SimStruct *S)
{
  SFc8_Assembly_QuadrotorInstanceStruct *chartInstance;
  ChartRunTimeInfo * crtInfo = (ChartRunTimeInfo *)utMalloc(sizeof
    (ChartRunTimeInfo));
  chartInstance = (SFc8_Assembly_QuadrotorInstanceStruct *)utMalloc(sizeof
    (SFc8_Assembly_QuadrotorInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc8_Assembly_QuadrotorInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.disableChart =
    sf_opaque_disable_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.mdlStart = mdlStart_c8_Assembly_Quadrotor;
  chartInstance->chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c8_Assembly_Quadrotor;
...
void c8_Assembly_Quadrotor_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c8_Assembly_Quadrotor(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c8_Assembly_Quadrotor(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c8_Assembly_Quadrotor(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c8_Assembly_Quadrotor_method_dispatcher.\n"
