...
function NatNetMatlabSample()
    ...
    try
        ...
        % compute necessary conditions beforehand and allocate memory
        quadNum = 1;
        N=2; 
        M=5; 
        m0=3;  
        ...
        C = C(2,:) ; D = D(2,:) ; % Estimate only velocity
        stored_pos = zeros(M-1, 3, quadNum);
                ...
                for idx = 1 : 1000   
                   ...
                   java.lang.Thread.sleep(5);  
                    ...
                    if(frameTime ~= lastFrameTime)
                        ...
                        lastFrameID = frameID;
                        nDifferentFrames = nDifferentFrames + 1;
                        % Parse mocap data
                        mocap_data = parseMocapData(data);
                        
                        for j = 1 : mocap_data.nRigidBodies
                            xc = mocap_data.pos(:,j);
                            v = C * stored_pos(:, :, j) + D * xc';
                            mocap_data.vel(:, j) = v';
                            stored_pos(:, :, j) = A * stored_pos(:, :, j) + B * xc';
                        end
                        
                     
                        % Add Controller here
                        quad_cmd = mocap2cmd(mocap_data, frameTime);
